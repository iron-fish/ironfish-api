...
export class KycController {
  ...
  async callback(
    ...
    const callback = await this.callbackService.create(dto);
    const transaction = await this.kycService.handleCallback(dto);
    if (transaction) {
      await this.callbackService.udpate(callback, transaction);
    }
